...
if (isset($_POST['update_blog'])) {
    ...
    $checkSlug = "SELECT id FROM blogs WHERE slug = '$slug' AND id != $id";
    $checkQuery = mysqli_query($conn, $checkSlug);

    if ($checkQuery && mysqli_num_rows($checkQuery) > 0) {
        $_SESSION["msg"] = '<div class="alert alert-info">This slug already exists. Please use a different slug.</div>';
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($fileTemp, $destination)) {
            ...
            if (!empty($blog['featured_image'])) {
                $oldImagePath = "image/" . $blog['featured_image'];
